...
class ChatTestController extends Controller
{
    ...
    {
        $model = ClientExpert::find()->alias('ce')
            ->joinWith('chatTopics t')
            ->where([
            'ce.client_id' => Yii::$app->user->identity->client->id
        ])
        ...
        if ($model) {
            foreach ($model as $clientEx) {
                ...
                $countChat = ChatModel::find()->andWhere([
                    'recipient_id' => $clientEx->client->user_id
                ])
                    ->andWhere([
                    'sender_id' => $clientEx->expert->user_id
                ])
                    ->andWhere([
                    'is_read' => 0
                ])
                    ->count();
                $countChat = $countChat ? $countChat : 0;

                $topic = ChatTopic::find()->where([
    ...
    {
        $topics = ChatTopic::find()->andWhere([
        ...
        if ($topics) {
            foreach ($topics as $topic) {
                ...
                $countChat = ChatModel::find()->where([
                    'topic_id' => $topic->id
                ])
                    ->andWhere([
                    'recipient_id' => $topic->client->user_id
                ])
                    ->andWhere([
                    'is_read' => 0
                ])
    ...
    public function actionGetTopics()
    {
        ...
        $topics = ChatTopic::find()->where([
            'client_expert_id' => $client_expert_id
        ])
            ->andWhere([
            'client_id' => $client_id
        ])
